...
pub fn render_sidebar_content(renderer: &mut Renderer, app: &App, _vertices: &mut Vec<Vertex>) {
    ...
    // Section 2: Insights (pinned shards) – item_height 35 to match sidebar hit-test
    ...
    for (section_idx, y_offset) in layout {
        ...
        match section_idx {
            ...
            2 => render_insights_section(app, &sidebar_rect, y_offset, section, renderer, padding),
            _ => {}
        }
    }
    
    // Pop sidebar_content parent
    renderer.pop_parent();
}
...
fn render_conversations_section(
    ...
    if app.sidebar.conversations_list.scroll_view.selection_border_visible || app.sidebar.conversations_list.scroll_view.selection_border_opacity > 0.01 {
        ...
        let mut white_border = Vec4::new(1.0, 1.0, 1.0, border_width); // border width in alpha
        white_border.w *= opacity; // Fade border in/out
    ...
    for (i, conv) in app.chat_state.conversations.iter().enumerate() {
        if let Some(item_rect) = section.item_rect(sidebar_rect, y_offset, i, padding) {
            ...
            // Item title text - use Text component with scissor
            // Use unique component ID for each conversation item
            ...
            let mut text_vertices = Vec::new();
            title_text.render(renderer, app, &mut text_vertices, None);
            // Add text vertices to main buffer with scissor
            if !text_vertices.is_empty() {
        ...
        slider_effect: false,
                ...
                // Info button icon (centered in square button)
                renderer.queue_icon(
                    icon_names::PENCIL,
                    Vec2::new(
        ...
        slider_effect: false,
                ...
                // Delete button icon (centered in square button)
                renderer.queue_icon(
                    icon_names::TRASH,
                    Vec2::new(
            ...
            let mut handle_bg_color = Vec4::new(0.35, 0.35, 0.38, 0.8);
...
fn render_documents_section(
    ...
    if app.sidebar.documents_list.scroll_view.selection_border_visible || app.sidebar.documents_list.scroll_view.selection_border_opacity > 0.01 {
        ...
        let border_width = 1.5;
        let mut white_border = Vec4::new(1.0, 1.0, 1.0, border_width);
        white_border.w *= opacity;
        let border_quad = Quad {
            position: selection_rect.position(),
            size: selection_rect.size(),
            color: white_border,
    ...
    for (i, insight) in app.insights_state.insights.iter().enumerate() {
        if let Some(item_rect) = section.item_rect(sidebar_rect, y_offset, i, padding) {
            ...
            let item_color = if is_selected {
                ...
                Vec4::new(0.2, 0.2, 0.23, 0.9)
            } else {
                Vec4::new(0.18, 0.18, 0.20, 0.6)
            };
            let item_bg = Quad {
