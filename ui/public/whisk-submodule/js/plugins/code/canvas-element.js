    ...
    redrawCanvas() {
        ...
        this.elements.forEach((element, index) => {
            ...
            if (element.type === 'path') {
                ...
                const fontSize = Math.max(10, element.thickness * 2); // World-space size
                this.ctx.fillStyle = element.color;
                this.ctx.textAlign = 'left';
                this.ctx.textBaseline = 'top';

                // Keep text sharp when zoomed in: rasterize at screen size so we don't scale up a small raster.
                // When zoom > 1 we draw in unzoomed space at (element * zoom) with font (fontSize * zoom).
                if (this.zoom > 1) {
                    this.ctx.save();
                    this.ctx.scale(1 / this.zoom, 1 / this.zoom);
                    this.ctx.font = `${fontSize * this.zoom}px sans-serif`;
                    this.ctx.fillText(element.text, element.x * this.zoom, element.y * this.zoom);
                    this.ctx.restore();
                } else {
